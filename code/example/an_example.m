...
[MHW,mclim,m90,mhw_ts]=detect(sst_full,datenum(1982,1,1):datenum(2016,12,31),datenum(1982,1,1),datenum(2011,12,31),datenum(1982,1,1),datenum(2016,12,31));
disp('saving')
%% 2.Tranfer the MHWs to masks (mask.m)
mask_daily = zeros(32,32,datenum(2016,12,31)-datenum(1982,1,1)+1);
d = 0;
...
for day_i = datenum(1982,1,1):datenum(2016,12,31)
    d = d+1;
    search_day = day_i;
    mask = zeros(32,32);
...
for k = 1:datenum(2016,12,31)-datenum(1982,1,1)+1
    ...
    save(['features_train',num2str(k),'.mat'],'features_train');
    save(['labels_train',num2str(k),'.mat'],'labels_train');
    disp(k)
end
%% Go to knn_test.py for KNN algorithm
% If end 
% put all of the mask_new to a file
% then, go next
...
% mask_daily(mask_daily==1 & sst==1)=0; % sst is the map including the land
% with 1 and ocean with 0. It can be made by yourselves :)
...
% This part is very long, you can check it in mhw_overlap_tracks.m
